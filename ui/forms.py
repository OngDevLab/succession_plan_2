...
def display_incumbent_form():
    """Display incumbent form in an expandable container instead of a dialog"""
    ...
    if is_editing:
        ...
        form_title = f"✏️ Edit: {person['PREFERRED_NAME_FIRST_NAME']} {person['PREFERRED_NAME_LAST_NAME']}"
    else: 
        person = st.session_state.selected_person
        plan_details = {}
        form_title = f"📝 New Plan: {person['PREFERRED_NAME_FIRST_NAME']} {person['PREFERRED_NAME_LAST_NAME']}"
    ...
    # Use an expander instead of dialog
    with st.expander(form_title, expanded=True):
        # Add a close button at the top
        col_close, col_spacer = st.columns([1, 10])
        with col_close:
            if st.button("✕ Close", key="close_incumbent_form", help="Close this form"):
        ...
        # Initialize session state more carefully to prevent calendar auto-open
        if contract_date_key not in st.session_state:
            # For new forms (not editing), always start with None to prevent calendar popup
            # For editing forms, only set date if there's actually a meaningful date
            if is_editing and contract_end_date_str:
        ...
        with col_date:
            # Always use None as default for new selections to prevent calendar auto-open
            date_value = st.session_state[contract_date_key]
            if date_value is not None:
                contract_end_date = st.date_input("Contract End Date (optional):", value=date_value, key=f"inc_date_input_{contract_date_key}")
            else:
                contract_end_date = st.date_input("Contract End Date (optional):", key=f"inc_date_input_{contract_date_key}")
...
def display_successor_form():
    """Display successor form in an expandable container instead of a dialog"""
    ...
    if is_editing:
        ...
        person = st.session_state.selected_person
        assessment = {} 
        form_title = f"Add: {person['PREFERRED_NAME_FIRST_NAME']} {person['PREFERRED_NAME_LAST_NAME']}"
    ...
    with st.expander(form_title, expanded=True):
        ...
        col_close, col_spacer = st.columns([1, 10])
        with col_close:
            if st.button("✕ Close", key="close_successor_form", help="Close this form"):
                st.session_state.selected_person = None
        ...
        with col_date:
            ...
            date_value = st.session_state[contract_date_key]
            if date_value is not None:
                contract_end_date = st.date_input("Contract End Date (optional):", value=date_value, key=f"date_input_{contract_date_key}")
            else:
                contract_end_date = st.date_input("Contract End Date (optional):", key=f"date_input_{contract_date_key}")
            # Update session state when date changes
            st.session_state[contract_date_key] = contract_end_date
