...
# Temporary access reference list, to be replaced by security table referencing, or LANID referencing via underlying tables.
user_access_list = ['Andy.Andrews@disney.com','Will.Ong.-ND@disney.com', 'Laurne.Jones@disney.com', 'Mackenzie.G.Whitman@disney.com','Sheri.Sebilian@disney.com','Rebecca.Alimena@abc.com','James.Biddle@disney.com','Jody.Morgan@disney.com','Grant.E.Gemelli@disney.com','Christopher.Bailey@disney.com','Omar.Jennings@disney.com','Shane.Dolan@disney.com','Winnie.Phung@disney.com','Lara.X.Willson@disney.com']

def extract_first_name(name_string):
    """
    Extracts the first name from a string formatted as:
    - '<last_name>, <first_name>'
    - '<last_name>, <first_name> -ND'

    Args:
        name_string: The full name string.

    Returns:
        The extracted first name as a string, or None if the format is incorrect.
    """
    try:
        # Split the string by the comma to separate last name from the rest
        parts = name_string.split(',')

        # The first name part is the second element in the list
        first_name_part = parts[1]

        # Clean up any leading/trailing whitespace
        first_name_part = first_name_part.strip()

        # Check if the "-ND" suffix exists and remove it
        # We split by space and take the first element to handle cases like "Jane -ND"
        if ' -ND' in first_name_part:
            first_name = first_name_part.split(' -ND')[0]
        else:
        ...
        # This will happen if the string doesn't contain a comma
        print(f"Error: The string '{name_string}' is not in the expected format.")
        return None
...
def main():
    ...
    if st.session_state.app_data['incumbent'] and st.session_state.app_data['successors']:
        ...
        #         use_container_width=True
        #     )
@st.dialog("Acknowledge", dismissible=False)
def acknowledge():
    with st.container():
        st.markdown("""
        ### Acknowledgement Required
        I understand the data I am accessing may be market-moving, and it is my responsibility to keep it completely secure and confidential.  
        For questions, please contact **Rebecca Alimena** from the Talent Management team or your **LTS Partner**.
        """)
        if st.button("I Acknowledge"):
            ...
            st.stop()  # Prevent rest of the app from loading until acknowledge
...
if not st.user.is_logged_in:
    ...
    if st.sidebar.button("Log out"):
        st.logout()
        st.stop()
    if st.user.email not in user_access_list:
        st.write(f"Sorry! It doesn't look like {st.user.email} has been granted access to this page, please contact People Insights to request access.")
    else:
        # Initialize session state
        if "acknowledged" not in st.session_state:
            st.session_state.acknowledged = False

        # Show acknowledgement card only if not yet acknowledged
        if not st.session_state.acknowledged:
            acknowledge()
        else:
            st.success(f"Welcome, {st.user.email}")
            st.sidebar.title(f"Welcome {extract_first_name(st.user.name)}!")
            main()
